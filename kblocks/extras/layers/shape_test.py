...
class ShapeTest(tf.test.TestCase):
    ...
    def test_flatten_leading_dims(self):
        x = inp((2, 3, 4))
        self.assertShapeEqual(shape_layers.flatten_leading_dims(x, 1),
                              (2, 3, 4))
        ...
        self.assertShapeEqual(shape_layers.flatten_leading_dims(x, 1),
                              (None, None, 4))
        self.assertShapeEqual(shape_layers.flatten_leading_dims(x, 2),
                              (None, 4))
    ...
    def test_reshape_leading_dim(self):
        x = inp((10, 3))
        self.assertShapeEqual(shape_layers.reshape_leading_dim(x, (2, 5)),
                              (2, 5, 3))
        self.assertShapeEqual(shape_layers.reshape_leading_dim(x, (5, -1)),
                              (5, 2, 3))

        x = inp((None, 3))
        self.assertShapeEqual(shape_layers.reshape_leading_dim(x, (-1, 2)),
                              (None, 2, 3))
        self.assertShapeEqual(shape_layers.reshape_leading_dim(x, (5, 2)),
                              (5, 2, 3))
    ...
    def test_reshape_ragged_leading_dim(self):
        ...
        np.testing.assert_equal(self.evaluate(out.values.row_splits),
                                [0, 50, 80, 90, 100])
    ...
    def test_as_batched(self):
        ...
        self.assertShapeEqual(shape_layers.as_batched(x, element_size=2),
                              (5, 2, 3))
