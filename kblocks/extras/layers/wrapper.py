...
def is_keras_tensor(x):
    return isinstance(x, (tf.Tensor, tf.Variable, tf.SparseTensor,
                          tf.RaggedTensor))  #and hasattr(x, '_keras_history')
...
def as_lambda(fn, *args, **kwargs):
    ...
    is_tensor = np.array([is_keras_tensor(x) for x in flat_args],
                         dtype=np.uint8)
    ...
    print(keras_args)
    # raise Exception()
    layer = tf.keras.layers.Lambda(_as_lambda,
                                   arguments=dict(structure=structure,
