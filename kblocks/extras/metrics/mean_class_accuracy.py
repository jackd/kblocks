...
class MeanClassAccuracy(tf.keras.metrics.Metric):
    def __init__(self, num_classes, name="mean_class_acc", dtype=tf.float32):
        ...
        self.total = self.add_weight(
            "total", shape=shape, initializer=tf.zeros_initializer, dtype=self.dtype
        )
        self.correct = self.add_weight(
            "correct", shape=shape, initializer=tf.zeros_initializer, dtype=self.dtype
        )
    ...
    def reset_states(self):
        K.batch_set_value(
            [
                (v, np.zeros(v.shape, dtype=v.dtype.as_numpy_dtype))
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        if num_classes != self.num_classes:
            raise ValueError(
                ...
                "but y_pred has shape {}".format(self.num_classes, y_pred.shape)
            )
        ...
        total_inc = tf.math.unsorted_segment_sum(sample_weight, y_true, num_classes)
