...
class CacheManager(abc.ABC):
    ...
    def __call__(self, dataset: tf.data.Dataset,
                 preprocess: bool = False) -> tf.data.Dataset:
        raise NotImplementedError('Abstract method')
...
class ParentManager(CacheManager):
    ...
    def __init__(self,
        ...
        self._managers = tuple(
            manager_impl(
                os.path.join(cache_dir, f'repeat-{i:04d}-{num_children:04d}'),)
...
class ShardedCacheManager(ParentManager):
    ...
    def _children(self, dataset):
        return [
            dataset.shard(self._num_shards, i) for i in range(self._num_shards)
        ]
