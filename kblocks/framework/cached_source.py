        ...
        return tfds.core.DatasetInfo(builder=self,
                                     features=tfds.core.features.FeaturesDict(
                                         tf.nest.map_structure(
                                             spec_to_feature,
                                             self.builder_config.example_spec)))
        ...
        with tf.keras.backend.learning_phase_scope(training):
            ...
            for s in tf.nest.flatten(spec):
                if s.shape.ndims is None:
                    raise ValueError(
                        'All specs must have number of dimensions known.')
            if not isinstance(spec, collections.Mapping):
                raise ValueError(f'map function must return a dict of tensors, '
                                 'got spec {spec}')
