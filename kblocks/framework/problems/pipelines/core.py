    ...
    def processed_spec(self, example_spec):
        dataset = tf.data.Dataset.from_generator(
            ...
            output_shapes=tf.nest.map_structure(lambda x: x.shape,
                                                example_spec))
        return self(dataset).element_spec
...
class BasePipeline(DataPipeline):
    ...
    def cache_path(self, training) -> str:
        return '' if self._cache_dir is None else os.path.join(
            self._cache_dir, 'cache', 'train' if training else 'val')
    ...
    def processed_spec(self, example_spec):
        dataset = tf.data.Dataset.from_generator(
            ...
            output_shapes=tf.nest.map_structure(lambda x: x.shape,
                                                example_spec))
        return self._process_dataset(dataset, spec_only=True).element_spec

    def __call__(self, dataset: tf.data.Dataset):
        return self._process_dataset(dataset, spec_only=False)
    ...
    def _process_dataset(self, dataset: tf.data.Dataset,
        ...
        if self._pre_cache_map is not None:
            ...
            cache_path = self.cache_path(tf.keras.backend.learning_phase())
            root_dir = None if cache_path == '' else os.path.dirname(cache_path)
