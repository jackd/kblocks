...
class BaseSource(DataSource):
    def __init__(
        self, dataset_fn: Callable[[Split], tf.data.Dataset], examples_per_epoch, meta
    ):
...
class PipelinedSource(DataSource):
    ...
    def examples_per_epoch(self, split: str):
        return self._base_source.examples_per_epoch(split) // self._pipeline.batch_size
...
def run_data_source(
    ...
    if callback is None:
        ...
        for example in tqdm(source.get_dataset(split), total=source.examples_per_epoch):
            pass
    else:
