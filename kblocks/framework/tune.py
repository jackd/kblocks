...
def tune(model_fn: Callable,
         ...
         directory=None,
         project_name=None):
    tuner = tuner_fn(functools.partial(model_fn, **source.meta),
                     objective,
                     max_epochs,
                     directory=directory,
    ...
    train_steps, val_steps = (
        source.examples_per_epoch(split) for split in splits)
    tuner.search(x=train_ds,
                 ...
                 callbacks=[
                     tf.keras.callbacks.TensorBoard(
                         os.path.join(directory, 'summaries'))
                 ])
