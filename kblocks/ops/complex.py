...
def swish(x: tf.Tensor, threshold=-4.0):
    return tf.where(x < threshold, tf.zeros_like(x), x * tf.nn.sigmoid(x))


def modified_swish(x: tf.Tensor, threshold=-4.0):
    if x.dtype.is_complex:
        ...
        return tf.where(
            real < threshold, tf.zeros_like(x), tf.complex(real * sig, imag * sig)
        )
    else:
        return swish(x)
...
def softplus(x: tf.Tensor, threshold=5.0):
