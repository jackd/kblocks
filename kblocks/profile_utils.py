...
def _extract_nodes_recursive(multi_node: MultiGraphNodeProto
...
def extract_multi_nodes(multi_node: MultiGraphNodeProto
...
def summarize(profiles: Iterable[MultiGraphNodeProto],
              print_fn: Callable[[Any], Any] = print):
    ...
    for k in ('total_exec_micros', 'total_requested_bytes', 'total_peak_bytes',
              'total_residual_bytes', 'total_output_bytes'):
        print_fn(msg.format(k, np.mean([getattr(p, k) for p in profiles])))
    print_fn(
        msg.format('sum_residual_bytes',
                   np.mean([sum_residual_bytes(p) for p in profiles])))
